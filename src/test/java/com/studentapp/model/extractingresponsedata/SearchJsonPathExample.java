...
public class SearchJsonPathExample {
    ...
    public static void inIt() {
        ...
        response = given()
                .when()
                .get("/products")
                .then().statusCode(200);  // response.log().all();
    }
    ...
    @Test
    public void test004() {
        List<String> categoriesList= response.extract().path("data[0].categories");
    ...
    public void test005() {
        //Homework
        List<String> size= response.extract().path("data.size");
...
//Homework
        ...
        System.out.println("------------------StartingTest---------------------------");
        System.out.println("The names of the products are : "+product);
        System.out.println("------------------End of Test---------------------------");
    }
  ...
  List<HashMap<String, ?>> values = response.extract().path("data.findAll{it.name=='Duracell - AA Batteries (8-Pack)'}");
    ...
    @Test
    public void test011() {
        List<String> vehiclesPrice =  response.extract().path("data.findAll{it.name==~/.*Black/}.price");

        System.out.println("------------------StartingTest---------------------------");
        System.out.println("The prices of products whose name end with = Vehicles are: "+vehiclesPrice  );
        System.out.println("------------------End of Test---------------------------");
    }
...
//        int id1 = (Integer) productMap.get("id");
